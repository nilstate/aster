...
const research = await readJson("skill_research_report.json");
...
const checks = [
  ...
  ["research schema", research.schema === "runx.skill_research_report.v1" && Array.isArray(research.findings) && research.findings.length > 0],
  ...
  ["contribution schema", contribution.schema === "runx.skill_contribution.v1" && contribution.state === "contribution_ready"],
  ...
  ["feed event schema", feedEvent.lane === "skill-contribution" && feedEvent.metadata?.state === "contribution_ready"],
];
...
function parseArgs(argv) {
  const parsed = {
    artifactsDir: ".artifacts/skill-contribution",
